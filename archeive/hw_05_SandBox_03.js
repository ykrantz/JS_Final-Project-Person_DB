...
// edit action withput prompt-need to fix
...
function edit_person(arr, id, field_name, new_value) {
  // without prompt. with arguments
  
  // TODO:
  // check if inpur correct
  // TODO: to check if nedded to get from prompt. and if te do whlie
  field_name = String(field_name.toLowerCase());
  let can_edit_flag = true;
  if (
    !valid_field_name(Person, field_name) ||
    !check_if_can_edit_filed(field_name)
  ) {
  ...
  if (
    ...
    can_edit_flag = check_valid_string(new_value);
  } else if (field_name == "id"|| ) {
    can_edit_flag = check_valid_id(id);
    can_edit_flag == true &&
      (can_edit_flag = check_if_uniqe(person_data_base, "id", id));
    } else if (field_name == "parant_id") {
    // TODO: check that the function work
    can_edit_flag = check_valid_parent_id_to_edit(arr, id);
  }

  if (can_edit_flag == false) {
    console.log("No change was made");
    return false;
  }

  let old_value = arr[find_index_in_arr_object(arr, "id", id)][field_name];
  arr[find_index_in_arr_object(arr, "id", id)][field_name] = new_value;
  console.log(`Person id: ${id} was updated.\n
  the old value in property ${field_name} was:${old_value}.\n
  The new value is: ${new_value}`);
}
edit_person(person_data_base, "211111111", "l_name", "SSSS");
console.log("new");
console.table(person_data_base);
...
// *******************
function check_valid_parent_id_to_edit(arr, id){
  // TODO: fix the function and make it approproate for it
...
function valid_field_name(main_object, field_name) {
  let falg = true;

  flag = check_valid_string(field_name);
  try {
    if (field_name in new main_object() == false)
      throw "the field name does't exsist";
  } catch (err) {
    console.log(err);
    flag = false;
  }
...
// console.log("LLL");
// console.log(valid_field_name(Person, "isd"));
// console.log("SSSS");
// console.log("f_name" in new Person());
